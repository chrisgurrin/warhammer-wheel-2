...
import {
    SelectedItemStore,
    setSelectedItem,
} from '../../redux/reducers/selectedItem';
...
export const Wheel: React.FC<Props> = ({ items = [] }) => {
    const radius = 300;
    ...
    const selectedItem = useSelector(
        (state: SelectedItemStore) => state.selectedItem.value,
    );
    ...
    useEffect(() => {
        ...
        let segAngle = 0;
        const filtered = filterItems(items, filter.items);
    ...
    return (
        <div className="flex flex-col items-center gap-8">
            ...
            <div className="flex flex-col gap-4 items-center">
                <button
                    className="bg-zinc-700/50 rounded-lg px-8 py-3 hover:outline outline-primary w-32"
                    onClick={btnSpin_OnClick}
                >
                    Spin
                </button>
