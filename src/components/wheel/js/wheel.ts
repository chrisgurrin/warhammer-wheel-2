...
const getPickedItem = (items: string[]): PickedItem => {
    ...
    const offset = Math.floor(
        getScaledValue(Math.random(), 0, 1, 0, items.length),
    );
...
const playSpinAnimation = async (
    ...
    return new Promise<void>((resolve) =>
        setTimeout(() => {
            if (elapsed < duration) {
                playSpinAnimation(rotation, duration, start, draw).then(() =>
                    resolve(),
                );
            } else {
                resolve();
...
export const redraw = (
    ...
    window.requestAnimationFrame(() =>
        drawWheel(ctx, items, r, spinAngle, segAngle, theme),
    );
};
...
export const spin = async (
    ...
    const rotationAmount =
        deg2rad(numRotations * 360) + segAngle * pickedItem.offset;
